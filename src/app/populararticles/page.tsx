...
import { VIEW_PARTICLES_METADATA } from './metadata';
import { VerticalCard } from '@/components/card/VerticalCard';
import { Suspense } from 'react';
import { ArticleSkeleton } from './loading';
import { getPopularArticles } from '../actions';

export const metadata: Metadata = { ...VIEW_PARTICLES_METADATA };
...
export default function ViewPopulararticles() {
  return (
...
async function PopularArticlesSection() {
  const popularArticlesData = await getPopularArticles({ limit: 0 });
  if (!popularArticlesData?.length) {
    return <div>Error fetching popular articles</div>;
  }
  return (
    <section className="px-4 md:px-0">
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {popularArticlesData.map((card: Article) => (
